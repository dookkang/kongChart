 ...
 * ***************************************************
 */
SurfacePlotLib = function(container){
	var S = this;
    this.containerElement = container;
    
    this.redraw = function(){
        this.SurfacePlotLib.init();
        this.SurfacePlotLib.redraw();
    }
    this.isMouseDown = true;
};


SurfacePlotLib.prototype.draw = function(data, options, basicPlotOptions, glOptions,_canvas){
  ...
  //			this.SurfacePlotLib.reRender(data, glOptions);
  //			return;
  //		}
  //		else {
  			this.SurfacePlotLib.cleanUp();
  			this.containerElement.innerHTML = "";
  //		}
  
      startXAngle_canvas = this.SurfacePlotLib.currentXAngle_canvas;
      startZAngle_canvas = this.SurfacePlotLib.currentZAngle_canvas;
      var rotationMatrix = this.SurfacePlotLib.rotationMatrix;
  
	}
   
    this.SurfacePlotLib = new JSSurfacePlotLib(xPos, yPos, w, h, colourGradient, this.containerElement, 
 ...
 */
SurfacePlotLib.prototype.createFrames = function(newData, glOptions) {
	
	var currentData = this.SurfacePlotLib.data;
...
SurfacePlotLib.prototype.getChart = function(){
    return this.SurfacePlotLib;
}

SurfacePlotLib.prototype.cleanUp = function(){
    if (this.SurfacePlotLib == null) 
        return;
    
    this.SurfacePlotLib.cleanUp();
    this.SurfacePlotLib = null;
}
 ...
 * *********************************
 */
JSSurfacePlotLib = function(x, y, width, height, colourGradient, targetElement, fillRegions, tooltips, xTitle, yTitle, zTitle, renderPoints, backColour,
   axisTextColour, hideFlatMinPolygons, tooltipColour, origin, startXAngle_canvas, startZAngle_canvas, rotationMatrix, zAxisTextPosition, glOptions, data,_canvas){
    this.xTitle = xTitle;
    this.yTitle = yTitle;
    this.zTitle = zTitle;
    ...
    if (rotationMatrix != null && rotationMatrix != void 0) 
      ...
      mat4.rotate(this.rotationMatrix, degToRad(JSSurfacePlotLib.DEFAULT_X_ANGLE_WEBGL), [1, 0, 0]);
      mat4.rotate(this.rotationMatrix, degToRad(JSSurfacePlotLib.DEFAULT_Y_ANGLE_WEBGL), [0, 0, 1]);
    }
    ...
    this.mvMatrix = mat4.create();//mat4.create에 의해 만들어진 이 매트릭스에는 모두 0으로 비어있는 행렬입니다.
    this.pMatrix = mat4.create();
        ...
        else // Render not quite so shiny non-GL surface-plot.
        {
            ...
            if (renderPoints) {
                for (i = 0; i < data3ds.length; i++) {
                    ...
                    var dotSize = JSSurfacePlotLib.DATA_DOT_SIZE;
...
/*            console.log('reder');
    ...
    this.createTargetDiv = function(){
        ...
        targetDiv.id = id;
        targetDiv.className = "SurfacePlotLib";
    ...
    this.mvPushMatrix = function(SurfacePlotLib){
        var copy = mat4.create();
        mat4.set(SurfacePlotLib.mvMatrix, copy);
        SurfacePlotLib.mvMatrixStack.push(copy);
    };
    
    this.mvPopMatrix = function(SurfacePlotLib){
        if (SurfacePlotLib.mvMatrixStack.length == 0) {
            throw "Invalid popMatrix!";
        }
        
        SurfacePlotLib.mvMatrix = SurfacePlotLib.mvMatrixStack.pop();
    };
    ...
    this.drawScene = function(){
        ...
        mat4.perspective(3.7, this.gl.viewportWidth / this.gl.viewportHeight, 0.1, 100.0, this.pMatrix);
        mat4.identity(this.mvMatrix);
        
        //mat4.translate(this.mvMatrix, [0.0, -0.3, -19.0]);
        mat4.translate(this.mvMatrix, [0.1, -0.07, -19.0]);
        ...
        if (useLighting) {
            ...
            var lightingDirection = [0.1, 0.0, 0.0];
            ...
            this.gl.uniform3f(this.shaderProgram.directionalColorUniform, 1.2, 1.2, 1.2);
        }
    ...
    this.handleMouseMove = function(event, context){
        ...
        else // rotate
        {
            ...
            //mat4.translate(newRotationMatrix, [0, -dy, 0]);
    ...
    this.initCanvas = function(){
        canvas.className = "SurfacePlotLibCanvas";
    ...
    this.calculateZScale = function(){
        ...
        for (var i = 0; i <= ticks; i++) {
            ...
            if(rounded2dp == 0){
            	labels.push(-60);
            }else if(rounded2dp == 64){
            	labels.push(-40);
            }else if(rounded2dp == 128){
            	labels.push(-20);
            }else if(rounded2dp == 192){
            	labels.push(0);
            }else if(rounded2dp == 256){
            	labels.push(20);
            }
    ...
    this.createCanvas = function(_canvas){
    	//console.log('createCanvas'+_canvas);
    	if(_canvas == null){
 ...
 mouseButton1Up.y + (mouseButton1Down.y - e.y));
    ...
    this.calculateScale = function(e){
        lastMousePos = new Point(0, JSSurfacePlotLib.DEFAULT_SCALE / JSSurfacePlotLib.SCALE_FACTOR);
        
        if (mouseButton3Up == null) {
            mouseButton3Up = new Point(0, JSSurfacePlotLib.DEFAULT_SCALE / JSSurfacePlotLib.SCALE_FACTOR);
        }
 ...
 mouseButton3Up.y + (mouseButton3Down.y - e.y));
        ...
        scale = lastMousePos.y * JSSurfacePlotLib.SCALE_FACTOR;
        
        if (scale < JSSurfacePlotLib.MIN_SCALE) 
            scale = JSSurfacePlotLib.MIN_SCALE + 1;
        else 
            if (scale > JSSurfacePlotLib.MAX_SCALE) 
                scale = JSSurfacePlotLib.MAX_SCALE - 1;
        
        lastMousePos.y = scale / JSSurfacePlotLib.SCALE_FACTOR;
...
GLText = function(data3D, text, pos, angle, SurfacePlotLib, axis, align){
    this.shaderTextureProgram = SurfacePlotLib.shaderTextureProgram;
    this.currenShader = null;
    this.gl = SurfacePlotLib.gl;
    this.setMatrixUniforms = SurfacePlotLib.setMatrixUniforms;
    
    this.vertexTextureCoordBuffer = null;
    this.textureVertexPositionBuffer = null;
    this.textureVertexIndexBuffer = null;
    this.context2D = SurfacePlotLib.context2D;
    this.mvPushMatrix = SurfacePlotLib.mvPushMatrix;
    this.mvPopMatrix = SurfacePlotLib.mvPopMatrix;
    this.texture;
    this.text = text;
    this.angle = angle;
    this.pos = pos;
    this.SurfacePlotLib = SurfacePlotLib;
    this.textMetrics = null;
    this.axis = axis;
    this.align = align;
    
    this.setUpTextArea = function(){
        this.context2D.font = 'normal 12px Verdana';
    ...
    this.writeTextToCanvas = function(text, idx){
        ...
        var r = hexToR(this.SurfacePlotLib.axisTextColour);
        var g = hexToG(this.SurfacePlotLib.axisTextColour);
        var b = hexToB(this.SurfacePlotLib.axisTextColour);
...
GLText.prototype.draw = function(){
    this.mvPushMatrix(this.SurfacePlotLib);
    ...
    mat4.multiply(this.SurfacePlotLib.mvMatrix, rotationMatrix);
    ...
    this.setMatrixUniforms(this.currentShader, this.SurfacePlotLib.pMatrix, this.SurfacePlotLib.mvMatrix);
    ...
    this.mvPopMatrix(this.SurfacePlotLib);
};

/*
 * This class represents the axes for the webGL plot.
 */
GLAxes = function(data3D, SurfacePlotLib){
    this.shaderProgram = SurfacePlotLib.shaderAxesProgram;
    this.currenShader = null;
    this.gl = SurfacePlotLib.gl;
    this.numXPoints = SurfacePlotLib.numXPoints;
    this.numYPoints = SurfacePlotLib.numYPoints;
    this.data3D = data3D;
    this.setMatrixUniforms = SurfacePlotLib.setMatrixUniforms;
    ...
    this.initAxesBuffers = function(){
        ...
        // Minor axis lines
        var lineIntervalX = axisExtent / (this.SurfacePlotLib.glOptions.xTicksNum / 2);
        var lineIntervalY = axisExtent / (this.SurfacePlotLib.glOptions.yTicksNum / 2);
        var lineIntervalZ = axisExtent / (this.SurfacePlotLib.glOptions.zTicksNum / 2);
        
        var i = 0;
        
        // X-axis division lines
        for (var count = 0; count <= this.SurfacePlotLib.glOptions.xTicksNum; i += lineIntervalX, count++) {
            // X-axis labels.
            var labels = this.SurfacePlotLib.glOptions.xLabels;
            ...
            glText = new GLText(data3D, label, labelPos, 90, SurfacePlotLib, "x");
...
/*            // back wall x-axis divisions.
        ...
        for (var count = 0; count <= this.SurfacePlotLib.glOptions.yTicksNum; i += lineIntervalY, count++) {
            // Y-axis labels.
            var labels = this.SurfacePlotLib.glOptions.yLabels;
            var label = labels[this.SurfacePlotLib.glOptions.yTicksNum - count];
            ...
            glText = new GLText(data3D, label, labelPos, 0, SurfacePlotLib, "y");
            this.labels.push(glText);
...
/*            var avertices = [
            ...
            minorVertices = minorVertices.concat([1.0,  1.0,  0.0]);
            minorVertices = minorVertices.concat([-1.0,  1.0,  0.0]);
            minorVertices = minorVertices.concat([1.0, -1.0,  0.0]);
            minorVertices = minorVertices.concat([-1.0, -1.0,  0.0]);*/
...
/*            minorVertices = minorVertices.concat([axisOrigin[0], axisOrigin[1] - i, 0]);
        ...
        for (var count = 0; count <= this.SurfacePlotLib.glOptions.zTicksNum; i += lineIntervalZ, count++) {
            // Z-axis labels.
            var labels = this.SurfacePlotLib.glOptions.zLabels;
            var label = labels[count];
            ...
            var glText = new GLText(data3D, label, labelPos, 90, SurfacePlotLib, "z");
            this.labels.push(glText);
...
/*        var labelPos = {
        ...
        */
        var labelPos = {
                x: 0.5,
                y: yAxisEndPoint[1] - 1.10,
                z: yAxisEndPoint[2]
            };        
        var glText = new GLText(data3D, this.SurfacePlotLib.xTitle, labelPos, 0, SurfacePlotLib, "x", "centre");
        this.labels.push(glText);
...
/*        labelPos = {
        ...
        glText = new GLText(data3D, this.SurfacePlotLib.yTitle, labelPos, 90, SurfacePlotLib, "x", "centre");
        this.labels.push(glText);
        
        // Set up the main Z-axis label.
        //console.log('Set up the main Z-axis label');
        labelPos = {
            x: yAxisEndPoint[0],
            y: yAxisEndPoint[1] - 1,
            z: 0.5
        };
        glText = new GLText(data3D, this.SurfacePlotLib.zTitle, labelPos, 90, SurfacePlotLib, "z", "centre");
        this.labels.push(glText);
        ...
        this.squareVertexPositionBuffer.itemSize = 3;//행
        this.squareVertexPositionBuffer.numItems = 4;//열
...
GLAxes.prototype.draw = function(){
	//console.log('GLAxes.prototype.draw');
    ...
    // Minor axis lines
    this.gl.uniform3f(this.currentShader.axesColour, 0.9, 0.9, 0.9); // Set the colour of the minor axis grid lines.
    this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this.axesMinorVertexPositionBuffer);
...
/*    this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this.squareVertexPositionBuffer);
    this.gl.vertexAttribPointer(this.currentShader.vertexPositionAttribute, this.squareVertexPositionBuffer.itemSize, this.gl.FLOAT, false, 0, 0);
 ...
 */
GLSurface = function(data3D, SurfacePlotLib){
    this.shaderProgram = SurfacePlotLib.shaderProgram;
    this.currentShader = null;
    this.gl = SurfacePlotLib.gl;
    this.numXPoints = SurfacePlotLib.numXPoints;
    this.numYPoints = SurfacePlotLib.numYPoints;
    this.data3D = data3D;
    this.colourGradientObject = SurfacePlotLib.colourGradientObject;
    this.setMatrixUniforms = SurfacePlotLib.setMatrixUniforms;
    ...
    this.updateSurface = function(data){
        ...
        var colors = [];
        var vertexNormals = [];
        //console.log('updateSurface start:');
        for (i = 0; i < this.numXPoints - 1; i++) {
            for (j = 0; j < this.numYPoints - 1; j++) {
                // Create surface vertices.
...
/*                var rawP5 = data[j + (i * this.numYPoints) + this.numYPoints + 2];
                var rawP6 = data[j + (i * this.numYPoints) + 2];                
                */
                ...
                vertices.push(rawP4.az);
/*                
                vertices.push(rawP5.ax);
                vertices.push(rawP5.ay);
                vertices.push(rawP5.az);
                
                vertices.push(rawP6.ax);
                vertices.push(rawP6.ay);
                vertices.push(rawP6.az);*/                
                ...
                var rgb3 = this.colourGradientObject.getColour(rawP3.lz * 1.0);
                var rgb4 = this.colourGradientObject.getColour(rawP4.lz * 1.0);
/*                var rgb5 = this.colourGradientObject.getColour(rawP5.lz * 1.0);
                var rgb6 = this.colourGradientObject.getColour(rawP6.lz * 1.0);
                */
                colors.push(rgb1.red / 255);
                colors.push(rgb1.green / 255);
                colors.push(rgb1.blue / 255, 1.0);
                ...
                colors.push(rgb4.green / 255);
                colors.push(rgb4.blue / 255, 1.0);
/*                colors.push(rgb5.red / 255);
                colors.push(rgb5.green / 255);
                colors.push(rgb5.blue / 255, 1.0);
                colors.push(rgb6.red / 255);
                colors.push(rgb6.green / 255);
                colors.push(rgb6.blue / 255, 1.0); */               
...
/*                // Normal of triangle 3.
                v1 = [rawP4.ax - rawP1.ax, rawP4.ay - rawP1.ay, rawP4.az - rawP1.az];
                v2 = [rawP5.ax - rawP1.ax, rawP5.ay - rawP1.ay, rawP5.az - rawP1.az];
                var cp3 = vec3.create();
                cp3 = vec3.cross(v1, v2);
                cp3 = vec3.normalize(v1, v2);
   
                // Normal of triangle 3.
                v1 = [rawP5.ax - rawP1.ax, rawP5.ay - rawP1.ay, rawP5.az - rawP1.az];
                v2 = [rawP6.ax - rawP1.ax, rawP6.ay - rawP1.ay, rawP6.az - rawP1.az];
                var cp4 = vec3.create();
                cp4 = vec3.cross(v1, v2);
                cp4 = vec3.normalize(v1, v2);
                */
                ...
                vertexNormals.push(cp2[2]);
/*                
                vertexNormals.push(cp3[0]);
                vertexNormals.push(cp3[1]);
                vertexNormals.push(cp3[2]);
                vertexNormals.push(cp3[0]);
                vertexNormals.push(cp3[1]);
                vertexNormals.push(cp3[2]);
                vertexNormals.push(cp4[0]);
                vertexNormals.push(cp4[1]);
                vertexNormals.push(cp4[2]);
                vertexNormals.push(cp4[0]);
                vertexNormals.push(cp4[1]);
                vertexNormals.push(cp4[2]);  */         
...
GLSurface.prototype.draw = function(){
	if(!stop){
		 this.currentShader = this.shaderProgram;
		    ...
		    //console.log('GLSurface.prototype.draw2');
	}
...
JSSurfacePlotLib.DEFAULT_X_ANGLE_CANVAS = 47;
JSSurfacePlotLib.DEFAULT_Z_ANGLE_CANVAS = 47;
JSSurfacePlotLib.DEFAULT_X_ANGLE_WEBGL = -50;
JSSurfacePlotLib.DEFAULT_Y_ANGLE_WEBGL = -20;
JSSurfacePlotLib.DATA_DOT_SIZE = 5;
JSSurfacePlotLib.DEFAULT_SCALE = 350;
JSSurfacePlotLib.MIN_SCALE = 50;
JSSurfacePlotLib.MAX_SCALE = 1100;
JSSurfacePlotLib.SCALE_FACTOR = 1.4;
