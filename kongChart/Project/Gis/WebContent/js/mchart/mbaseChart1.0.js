var defaultOption = {data_arr:getSampleData(30),axes_font_size:8,left_margin:25,right_margin:20,top_margin:20,
			 yaxis_tik_cnt:16,tik_half_width:2,yaxis_tik_gep_value:4,xaxis_tik_cnt:64,xaxis_xstart:0,zoom:1,radius:0.5,
			 tooltip_rect_y: 30,summax:20,dot_length:3,x_label:'X',
			 y_label:'Y',//왼쪽 Y축 타이틀
			 y2_label:'Y2',//오른쪽 Y축 타이틀
			 x_label_font_weight:300,
			 x_label_font_size:12,x_label_font_name:'Arial',x_label_font_color:'rgba(192,97,69,1)',y_label_font_weight:500,//italic
			 y_label_font_size:12,y_label_font_name:'Arial',y_label_font_color:'rgba(74,142,187,1)',
			 x_value_font_weight:'',x_value_font_size:8,x_value_font_name:'Arial',x_value_font_color:'gray',
			 y_value_font_weight:'',y_value_font_size:8,y_value_font_name:'Arial',y_value_font_color:'gray'
			 ,tooltip_font_size:10,tooltip_font:' 9pt Arial'
			 ,title:'제목',yaxis_limit:30,data_type:3
			 ,ly_width:0.13 //왼쪽 Y 폭 
			 ,ry_width:0, //오른쪽 Y 폭
			 b_height:0.18,arrow:false,default_arrow_color:'rgba(186,204,202,1)',arrow_no1_color:'rgba(245,66,102,1)',
			 arrow_colors:'["rgba(255,255,0,1)","rgba(255,255,50,1)","rgba(255,255,100,1)","rgba(255,255,150,1)","rgba(255,255,200,1)"]',
			 axis_use_cnt:1,//Y축사용 1:한개,2:두개(왼쪽,오른쪽)
			 use_right_axis_only:false//오른쪽Y만 사용
			 ,_y_min:0//메트릭스의 y축의 최소값.
			 ,_y_max:0//메트릭스의 y축의 최대값.
			 ,_x_min:0//메트릭스의x축의 최소값.
			 ,_x_max:0//메트릭스의x축의 최대값.
			 ,y_div_cnt:0//메트릭스의y축 분할간격수
			 ,x_div_cnt:0
			 ,y_min_re:0//메트릭스의 y축의 최소값의 변환값.
			 ,y_max_re:0//메트릭스의 y축의 최대값의 변환값.
			 ,x_min_re:0//메트릭스의x축의 최소값의 변환값.
			 ,x_max_re:0//메트릭스의x축의 최대값의 변환값.
			 ,best:0//가장큰값 순위 베스트를 선정함.
			 ,box : false//접속함표시
			 ,tool_tip: false//툴팁박스표시
...
/*
	 	기준선(Y축기준의 부가기능)
	 		...
	 		- 속성
	 			alramColor:기준선의 색상
	 			alramLabel:기준선에 표시할 라벨
	 			alramValue:선을그을 Y좌표값
	 			s_draw:(true:선을그려라,false:선을숨겨라)	 
	 */
	 var defaultYBaseLines =
		 [
		    {alramColor:'rgba(192,97,69,1)',alramLabel:'실내온도레벨',alramValue:'4200',s_draw:false},
		    {alramColor:'rgba(74,142,187,1)',alramLabel:'선로온도레벨',alramValue:'2200',s_draw:false},
		    {alramColor:'rgba(156,203,104,1)',alramLabel:'선로온도HH1',alramValue:'5000',s_draw:false},
		    {alramColor:'rgba(130,179,215,1)',alramLabel:'선로온도HH2',alramValue:'10000',s_draw:false}
		 ];
	 /*
	 	변화율(X축기준의추가기능)
	 		- 복수개를 설정할수있다.
	 ...
	 */	
	 var defaultXTerms =
			 [
			    {termColor:'rgba(74,142,187,0.2)',termLabel:'실내온도 변화율',fromValue:'10',toValue:'20',s_draw:false},
			    {termColor:'rgba(130,179,215,0.2)',termLabel:'선로온도 변화율',fromValue:'30',toValue:'45',s_draw:false}
			 ];	
...
/*        var shiftPressed = 0;
    ...
    this.setReMinMax = function(yAxisObj){
		...
		var new_gepDecimal_dived;
		var y_tik_cnt = 5;
		if(M.canvas.height < 100){
			y_tik_cnt = 2;
		}else if(M.canvas.height < 200){
			y_tik_cnt = 3;
		}else if(M.canvas.height < 300){
			y_tik_cnt = 3;
		}else if(M.canvas.height < 400){
			y_tik_cnt = 4;
		}
		
		if(0 < Math.abs(yAxisObj.max - yAxisObj.min) && Math.abs(yAxisObj.max - yAxisObj.min) < 1){
			
			new_gepDecimal_dived = (yAxisObj.max-yAxisObj.min)/y_tik_cnt;
		}else{
			new_gepDecimal_dived = this.getSmartValue((yAxisObj.max-yAxisObj.min)/y_tik_cnt,'ceil');
		}
...
/*		if(new_gepDecimal_dived < 1){
		...
		*/
		...
		if(0 < Math.abs(M.yAxisObj.max - M.yAxisObj.min) && Math.abs(M.yAxisObj.max - M.yAxisObj.min) < 1){
			new_start_min = yAxisObj.min;
		}else{
			new_start_min = this.getSmartValue(start_min,'floor');
		}
		...
		if(this.type == 'metrix'){
			yAxisObj.min = this._y_min;
			yAxisObj.max = this._y_max;//yAxisObj.max++;		
			
			yAxisObj.min2 = this._y_min;
			yAxisObj.max2 = this._y_max;//yAxisObj.max2++;
			yAxisObj.div = (this._y_max-this._y_min)/this.y_div_cnt;
		}
	}
...
/*		if(0 < Math.abs(M.yAxisObj.max - M.yAxisObj.min) && Math.abs(M.yAxisObj.max - M.yAxisObj.min) < 1){
			this.yAxisObj.min = yAxisObj.min;
			this.yAxisObj.max = yAxisObj.max;
		}*/
	...
	this.getNaturalNumber = function(_obj,num) {
		var zcnt = new Object();
		zcnt.cnt = 0;
		var re = this.makeNumTenMulti(num,zcnt);
		re = Math.round(re);
		zcnt.value = num*Math.pow(10,re);
		zcnt.oriValue = num; 
		zcnt.tenSimbol = 'e-'+zcnt.cnt; 
		_obj.axisInfo = zcnt;
	...
	this.makeNumTenMulti = function(num,zcntObj) {
		var re = num*10;
		zcntObj.cnt++;
		if(0 < Math.abs(re) && Math.abs(re) < 1){
			re = this.makeNumTenMulti(re,zcntObj);
		}else{
		}
		return re;
	}
	...
	this.leftCanvasWidth = leftCanvas.width;
	this.leftCanvasHeight = leftCanvas.height;
...
/*		
	...
	function yInfoBox(pt){
		...
		if(pt > 0){
			...
			if(0 < Math.abs(yo.max - yo.min) && Math.abs(yo.max -yo.min) < 1){	
				_txt = (parseFloat(_txt)*Math.pow(10,yo.axisInfo.cnt)).toFixed(2)+yo.axisInfo.tenSimbol;
			}else{
			}
	...
	function crossLine(pt){
		...
		M.bufferCtx.beginPath();M.bufferCtx.strokeStyle = "red";M.setLine(M.bufferCtx,1,true);M.bufferCtx.moveTo(xp,M.yAxisObj.yStart);M.bufferCtx.lineTo(xp,0);M.setStroke(M.bufferCtx,true);
	...
	function xDetail(pt){
		...
		if(M.series[0].s_chartType == 'metrix'){
			...
			now_x_label2 = M.xAxisObj.min +t;
		}else{
			var aSeries = null;
			//maxIndex시리즈가 unchecked이면 x-label 맵이 없으므로 맵을 가지는 series를 찾는다.(tooltip표시를 위함)
			for(var g=0; g < M.series.length; g++){
				var a = M.series[g];
				if(aSeries == null){
					aSeries = a;
				}else{
					if(aSeries._dataArr_forAxis.length <   a._dataArr_forAxis.length ){
						aSeries = a;
					}
				}
			}
	    ...
	    if(M.type == 'metrix'){
	    	//now_x_label2 = (now_x_label2+1) * M.x_max_re/((M._x_max-M._x_min)+1);
	    	now_x_label2 = (now_x_label2+1) * M._x_max/((M._x_max-M._x_min)+1);
	    }
	    return now_x_label2;
	}
	...
	canvas.addEventListener('mousemove',function(evt){
		...
		if(M.type != 'metrix'){
			...
			pt.x = Math.floor(pt.x/M.xAxisObj.tikGepLength)*M.xAxisObj.tikGepLength;
			pt.x_center = pt.x+(M.xAxisObj.tikGepLength/2);
		}
		M.dragged = true;
		
		M.refreshNowBackground();
		//if(pt.x < 0 || pt.y > M.centerCanvas.height){
		if(M.isShiftPressed){
		}else{
			if(M.dragStart){
				drawBox(pt);
				...
				//if(M.type != 'metrix'){
					//console.log(x+':'+y);
					M.makeTooltip(pt,x,y);
				//}
				crossLine(pt);
			    
			}
			M.callbackDraw(false);
		}
	...
	function evt_click(evt){
		if(M.menu_visible){
		var pt = setEvent(evt);
		if(M.centerCanvas.width > pt.x && pt.x > M.centerCanvas.width-M.menuBoxWidth ){
			if(pt.y < M.menuBoxWidth ){
				if($(this.parentNode.parentNode.childNodes[1]).attr('show') == 'false'){
					var w = $(this.parentNode.parentNode.childNodes[0]).width();
					var chart_w = '';
					var option_w = '';
					if(w > 1000){chart_w = '80%';option_w = '20%';}
					else if(w > 800){chart_w = '70%';option_w = '30%';}
					else if(w > 600){chart_w = '60%';option_w = '40%';}
					else if(w > 400){chart_w = '60%';option_w = '40%';}
					else if(w > 300){chart_w = '40%';option_w = '60%';}
					else if(w > 200){chart_w = '30%';option_w = '70%';}
					else if(w > 100){chart_w = '20%';option_w = '80%';}
					$(this.parentNode.parentNode.childNodes[0]).width(chart_w);
					$(this.parentNode.parentNode.childNodes[1]).width(option_w);
					$(this.parentNode.parentNode.childNodes[1]).attr('show','true');
				}else{
					$(this.parentNode.parentNode.childNodes[0]).width('100%');
					$(this.parentNode.parentNode.childNodes[1]).width('0%');
					$(this.parentNode.parentNode.childNodes[1]).attr('show','false');
				}
				redraw(M.chartId);
			}
		}
	...
	canvas.addEventListener('click',evt_click,false);	
}
	...
	this.initView = function(){
		...
		M.bufferRightCtx.clearRect(0, 0, M.rightCanvas.width, M.rightCanvas.height);
		//M.bufferRightCtx.drawImage(M.bufferOriRightImage,0, 0);//안지우면 숫자가 굵게 나와요		
		
		if(M.bufferLeftImage != null && M.bufferLeftImage.width > 0){
		...
		if(M.bufferRightImage != null && M.bufferRightImage.width > 0){
			M.rightCtx.clearRect(0, 0, M.rightCanvas.width, M.rightCanvas.height);
			M.rightCtx.drawImage(M.bufferRightImage,0, 0);
		}
		...
		for(var k=0; k < M.series.length; k++){
			...
			seriesObj.objectMap = seriesObj.objectMapOri;
			seriesObj._dataArr_forAxis = null;	
			seriesObj._dataArr_forAxis2 = null;	
			if(seriesObj.s_chartType == 'metrix'){
				M.xAxisObj.min_pre = 0;
				M.xAxisObj.min = M._x_min;
				M.xAxisObj.max = M._x_max;
				M.yAxisObj.min = M._y_min;
				M.yAxisObj.max = M._y_max;
				M.yAxisObj.tikCnt = ((M._y_max-M._y_min)+1)/M.y_div_cnt;
	...
	function mouseup_out(evt,initx,inity){
		...
		if(M.dragStart != null && M.dragStart != 'undefined' ){
			...
			if(Math.abs(M.dragStart.y - M.dragEnd.y) > 10 || M.isShiftPressed){
				if(!M.isShiftPressed){
					if(v1> v2){
						if(0 < Math.abs(yo.max - yo.min) && Math.abs(yo.max - yo.min) < 1){
							yo.min = v2;
				...
				if(vx1Index> vx2Index){
					moving_index = vx1Index - vx2Index;
					if(!M.isShiftPressed){
						M.xAxisObj.min += Math.ceil(vx2Index);
						M.xAxisObj.max = M.xAxisObj.min+(Math.floor(vx1Index)-Math.ceil(vx2Index));	
					}else{
						var min_max_gep = M.xAxisObj.max-M.xAxisObj.min;
						M.xAxisObj.min += Math.ceil(moving_index);
						M.xAxisObj.max += Math.ceil(moving_index);
						if(M.xAxisObj.max > M.maxIndex){
							
							M.xAxisObj.max = M.maxIndex;
							M.xAxisObj.min = M.xAxisObj.max-min_max_gep;
						}
					}
				}else{
					moving_index = vx2Index - vx1Index;
					if(!M.isShiftPressed){
						M.xAxisObj.min += Math.ceil(vx1Index);
						M.xAxisObj.max = M.xAxisObj.min+(Math.floor(vx2Index)-Math.ceil(vx1Index));
					}else{
						var min_max_gep = M.xAxisObj.max-M.xAxisObj.min;
						M.xAxisObj.min -= Math.floor(moving_index);
						M.xAxisObj.max -= Math.floor(moving_index);
						if(M.xAxisObj.min < 0){
							M.xAxisObj.min = 0;
							M.xAxisObj.max = M.xAxisObj.min + min_max_gep;
						}
				...
				if(M.use_right_axis_only){
					M.bufferRightCtx.clearRect(0, 0, M.bufferRightImage.width, M.bufferRightImage.height);
				...
				if(M.metrix_export){
					try{
						metrix_export(M.xAxisObj.min,M.xAxisObj.max,yo.min,yo.max);
					}catch(e){
						alert('해당페이지에 metrix_export function을 추가하세요!');
					}
...
MbaseChart.prototype.takeAPic2 = function() {
	...
	if(this.use_right_axis_only || this.axis_use_cnt == 2){
...
//툴팁만들기
MbaseChart.prototype.makeTooltip= function(pt,__x,__y) {
	var _x;
	var _y;
	if(this.series[0].s_chartType == 'metrix'){
		if(__x != null){
			_x = Math.floor(__x);
		}
		if(__y != null){
	...
	if(this.tool_tip){
			...
			for(var k=0; k < this.series.length; k++){
				...
				if(seriesObj.objectMap != null){
					var aObject = null;
					aObject = seriesObj.objectMap[_x];
					if(aObject != null){
						flg = true;	
					}
		/*			for(var i=0; i < seriesObj.objectArr.length; i++){
						...
						if(seriesObj.s_chartType == 'spline' || seriesObj.s_chartType == 'line'){
		
							flg = this.hitTestCircle(pt.x,pt.y,aObject,this.canvas.width/2,this.canvas.height/2);
							if(flg){
								break;
							}
						}else if(seriesObj.s_chartType == 'bar'){
							flg = this.hitTestRect(pt.x,pt.y,aObject,this.canvas.width/2,this.canvas.height/2);
							if(flg){
								break;
							}						
						}
					...
					if(flg){
						var tooltip_obj = {};
						if(seriesObj.s_chartType == 'metrix'){
							var v = aObject.lx_label;
	
							tooltip_obj.text = seriesObj.s_name+ " ("+this.comma(v)+" : "+this.comma(aObject.ly)+",Z:"+this.comma(aObject.value);
						...
						if(seriesObj.s_chartType == 'bar'){
							this.bufferCtx.beginPath();
							this.bufferCtx.fillStyle = 'yellow';
							var _x = aObject.x-(this.xAxisObj.tikGepLength-2)/2;
							var _y = aObject.y;					
							this.bufferCtx.fillRect(_x, _y,(this.xAxisObj.tikGepLength-2),this.centerCanvas.height-aObject.y );
							this.bufferCtx.fill();
							this.bufferCtx.stroke();
							this.bufferCtx.closePath();	
						}else{
							this.bufferCtx.beginPath();
							this.bufferCtx.fillStyle = seriesObj.s_color;
							//this.bufferCtx.arc(aObject.x, aObject.y,6, 0, 2 * Math.PI, false);
							this.bufferCtx.fill();
							this.bufferCtx.stroke();
							this.bufferCtx.closePath();	
						}
			...
			this.tooltip_rect_y = this.tooltip_font_size*text_arr.length+tooltipMargin*3+ tooltipLineMargin*3*(text_arr.length-1);
			
			var ttp = this.getToolTipPosition(pt.x,pt.y,this.tooltipRectX,this.tooltip_rect_y,tooltipMargin);
			
			var tx = ttp.rect_start_x;
			var ty = ttp.rect_start_y;
			var tw = this.tooltipRectX+tooltipMargin;
			var th = this.tooltip_rect_y+tooltipMargin;
		...
		if((this.series[0].s_chartType == 'metrix' && (_x > this._y_min-1))
				|| this.series[0].s_chartType != 'metrix'){
			this.bufferCtx.beginPath();
			this.bufferCtx.fillStyle = "rgba(255,255,255,1)";
			var boxHeight = this.menuBoxWidth;
			this.bufferCtx.fillRect(0,0,this.centerCanvas.width,boxHeight);
			this.bufferCtx.fill();
			this.bufferCtx.closePath();
		}
		this.bufferCtx.font = this.tooltip_font;
		...
		var top_margin = 3;
		var left_margin = 3;
		for(var k=0,x=left_margin; k < this.series.length; k++){
			var seriesObj = this.series[k];
			if(seriesObj.s_chartType == 'metrix'){
/*				this.bufferCtx.fillStyle = seriesObj.s_color;
				this.bufferCtx.fillRect(x,top_margin,box_width,box_width);
				this.bufferCtx.strokeStyle = 'black';
				this.bufferCtx.strokeRect(x,top_margin,box_width,box_width);*/
				if(_x > this._y_min-1 ){
					var aObject = seriesObj.objectMap[_x+'x'+_y];
					x += box_width+left_margin;
					//툴팁강화
					if(aObject != null && typeof aObject.c != 'undefined'){
						this.bufferCtx.fillStyle = aObject.c;
						this.bufferCtx.fillRect(x,top_margin,box_width,box_width);
						this.bufferCtx.strokeStyle = 'black';
						this.bufferCtx.strokeRect(x,top_margin,box_width,box_width);
						x += box_width+left_margin;
						...
						//여기
						var txt = "(X:"+this.metrix_reval_x(aObject.ix)+",Y:"+this.metrix_reval_y(aObject.iy)+",값:"+aObject.v+")";
						this.bufferCtx.fillText(
								txt,
								x,
								this.tooltip_font_size+top_margin);	
					}else{
				...
				if(seriesObj != null && seriesObj.objectMap != null){
					var aObject = seriesObj.objectMap[_x];
					...
					if(aObject != null){
						...
						x += box_width+left_margin;
						this.bufferCtx.fillStyle = 'black';
						var txt = seriesObj.s_name+ " ("+this.comma(aObject.lx_label)+" : "+this.comma(aObject.ly)+")";

						this.bufferCtx.fillText(
								txt,
								x,
								this.tooltip_font_size+top_margin);
						x += this.bufferCtx.measureText(txt).width+space;
						if(seriesObj.s_chartType == 'bar'){
							
							this.bufferCtx.beginPath();
							this.bufferCtx.fillStyle = 'yellow';
							var _xx = aObject.x-(this.xAxisObj.tikGepLength-2)/2;
							var _yy = aObject.y;					
							this.bufferCtx.fillRect(_xx, _yy,(this.xAxisObj.tikGepLength-2),this.centerCanvas.height-aObject.y );
							this.bufferCtx.fill();
							this.bufferCtx.stroke();
							this.bufferCtx.closePath();	
							...
							this.bufferCtx.beginPath();
							this.bufferCtx.fillStyle = seriesObj.s_color;
							if(seriesObj.s_shape == 'circle'){
								this.bufferCtx.arc(aObject.x, aObject.y,6, 0, 2 * Math.PI, false);
							}else if(seriesObj.s_shape == 'rectangle'){
								this.bufferCtx.fillRect(aObject.x-10/2, aObject.y-10/2,10,10 );
							}
							this.bufferCtx.closePath();	
							this.bufferCtx.fill();
							this.bufferCtx.stroke();	
	...
	if(this.menu_visible){
		var bw_margin = 2;
		var y = bw_margin;
		var bh = boxHeight*0.8;
		var bw = bh-bw_margin*2;		
		this.bufferCtx.beginPath();
		this.bufferCtx.fillStyle = 'rgba(207,207,207,1)';
		this.bufferCtx.fillRect(this.centerCanvas.width-bh-2,1,bh,bh);
		this.bufferCtx.fill();
		
		this.bufferCtx.fillStyle = 'rgba(50,50,50,1)';
		this.bufferCtx.fillRect(this.centerCanvas.width-bh,y,bw,bh/6);
		this.bufferCtx.fill();
		y += bh/4+bw_margin/2;
		this.bufferCtx.fillStyle = 'rgba(50,50,50,1)';
		this.bufferCtx.fillRect(this.centerCanvas.width-bh,y,bw,bh/6);
...
MbaseChart.prototype.drawYAxis_common = function(centerObj,xAxisObj,yAxisObj,dataArr,idDrawAxis,_type) {
    ...
    if(this.ry_width > 0  && this.axis_use_cnt == 2){
	...
	if((this.axis_use_cnt == 2 || this.use_right_axis_only)){
	...
	if(this.ly_width > 0){
		...
		if(this.axis_use_cnt == 2){
			...
			this.bufferLeftCtx.fillStyle = this.y_label_font_color;;
		}
	...
	if(this.y_base_lines != null){
		
		for(var i=0; i < this.y_base_lines.length; i++){
			var addInfoObj = this.y_base_lines[i];
			if(addInfoObj.s_draw){
				var tmp_alram_y;
				if(this.use_right_axis_only){
	...
	var cell_height = Math.abs(this.toPhysicalVal(1)-this.toPhysicalVal(2));
	if(!this.use_right_axis_only){
		for(var i=yAxisObj.min2; i < (yAxisObj.max2+1); i+=yAxisObj.div){
			...
			if(this.type == 'metrix' || (this.type != 'metrix' && pi > this.yaxis_limit)){
				...
				if(this.type == 'metrix'){
					...
					if(0 < Math.abs(this.yAxisObj.max - this.yAxisObj.min) && Math.abs(this.yAxisObj.max - this.yAxisObj.min) < 1){	
						this.getNaturalNumber(yAxisObj,i);
						la = (i*Math.pow(10,yAxisObj.axisInfo.cnt)).toFixed(2)+yAxisObj.axisInfo.tenSimbol;		
					}else{
				...
				if((this.axis_use_cnt == 2 || this.use_right_axis_only)){
					...
					if(this.ly_width > 0){
					    ...
					    if(this.type == 'metrix'){
						    this.bufferLeftCtx.moveTo(this.leftCanvas.width-yAxisObj.tikHalfWidth,pi-cell_height/2);
						    this.bufferLeftCtx.lineTo(this.leftCanvas.width+yAxisObj.tikHalfWidth,pi-cell_height/2);
						}else{
				...
				if(this.ly_width > 0){
				    if(this.type == 'metrix'){
				    	this.bufferLeftCtx.fillText(la,this.leftCanvas.width-mwidth-yAxisObj.tikHalfWidth*2,pi+correction-cell_height/2);
					}else{
		...
		for(var i=this.yAxisObj2.min2; i < (this.yAxisObj2.max2+1); i+=this.yAxisObj2.div){
			...
			if(this.type == 'metrix' || (this.type != 'metrix' && pi > this.yaxis_limit)){
				...
				if(this.type == 'metrix'){
					...
					if(0 < Math.abs(this.yAxisObj2.max - this.yAxisObj2.min) && Math.abs(this.yAxisObj2.max - this.yAxisObj2.min) < 1){	
						this.getNaturalNumber(this.yAxisObj2,i);
						la = (i*Math.pow(10,this.yAxisObj2.axisInfo.cnt)).toFixed(2)+this.yAxisObj2.axisInfo.tenSimbol;
					}else{
						la = this.comma(this.procK(i));
					}
				}
				...
				if((this.axis_use_cnt == 2 || this.use_right_axis_only)){
					if(this.ry_width > 0){
						this.bufferRightCtx.beginPath();
					    this.bufferRightCtx.font = this.y_value_font;
					    this.bufferRightCtx.moveTo(0,pi);
					    this.bufferRightCtx.lineTo(this.yAxisObj2.tikHalfWidth,pi);
					    this.bufferRightCtx.stroke();
					}
				}else{
...
MbaseChart.prototype.setZoomData = function(seriesArr,xAxisObj,isCeil,availLog,isHalf) {
	...
	for(var i=0;i < seriesArr.length; i++){
		...
		if(mainArr == null){
			...
			this.maxIndex = mainArr.length-1;
	...
	if(this.zoom == 1){
		this.xAxisObj.min = 0;
		this.xAxisObj.max = this.maxIndex;
...
/*		for(var k=0; k < mainArr.length; k++){
	...
	if(this.half_x_axis){
		...
		xAxisObj.tikGepLength = ((cw/(this.left_indexs-1)));//0占쏙옙1揶쏆뮆以�占썬끉堉긷첎占쏙옙嚥∽옙
		xAxisObj.tikGepLength2 = ((cw/(_dataArr2.length-this.left_indexs)));
	}else{
...
MbaseChart.prototype.drawXAxis_spline_draw = function(centerObj,xAxisObj,yAxisObj,series_arr,isDrawXAxis,z,_dataArr2,index) {
	if(series_arr[z].s_draw){
		...
		if(series_arr[z].s_chartType == 'bar' || ( series_arr[z].s_draw)){
			var t_ly;
			...
			if(this.type == 'metrix'){
				if(true){
					for(var e=0; e < _dataArr2[index][z].length; e++){
						...
						this.drawPoint(centerObj,xAxisObj,yAxisObj,series_arr,isDrawXAxis,z,_dataArr2,index,circle_size,t_ly,colorText,_dataArr2[index][z][e]);					
					}	
				}
			}else{
				var yo;
		...
		if(this.box && _dataArr2[index][z][2] == 1 && series_arr[z].s_chartType != 'bar'){
			this.bufferCtx.fillStyle =  series_arr[z].s_boxColor;
			this.bufferCtx.fillRect(index*xAxisObj.tikGepLength-(this.boxWidth*2/3)/2, this.toPhysicalVal(_dataArr2[index][z][1])-this.boxWidth/2,(this.boxWidth*2/3),this.boxWidth);	
		}			
	}
...
//좌표값 그리기
MbaseChart.prototype.drawPoint = function(centerObj,xAxisObj,yAxisObj,series_arr,isDrawXAxis,z,_dataArr2,index,circle_size,t_ly,color,metrixValue) {
	var t_i =  index;
	var t_lx =  index;
	var t_lx_label;
	...
	try{
		...
		console.dir(index+':ERROR z:'+z+':'+_dataArr2[index][z]);
	}
	
	var t_x = index*xAxisObj.tikGepLength;
	if(this.half_x_axis && index > this.left_indexs-1){
	...
	var t_r = this.xAxisObj.tikGepLength/2;
	var t_s = 0;
	var t_e = 2 * Math.PI;	
	//metrix log!
	//console.log(z+' series:'+index+' x/'+t_ly+' yIndex/'+metrixValue+' value');
	var arcObject = {
			'x':t_x, //x축 물리좌표값
			'y':t_y, //y축 물리좌표값
			'r':t_r, //한포인트를 구성할 원의 반지름
			's':t_s, //한포인트를 구성할 원의 시작 라디안각
			'e':t_e, //한포인트를 구성할 원의 끝 라디안각
			'i':t_i, //x축 인덱스
			'ix':index+this.xAxisObj.min, //x축 인덱스(메트릭스에서사용)
			'iy':t_ly, //y축 인덱스(메트릭스에서사용)
			'v':metrixValue, //메트릭스value(메트릭스에서사용)
			'c':color, //메트릭스컬러rgba값
			'z':z, //시리즈번호
			'lx':t_i, //x논리값
			'lx_label':t_lx_label,//x라벨값
			'ly':t_ly,//y논리값
			'shape':series_arr[z].s_shape//한포인트의 모양
	};		

	if(series_arr[z].s_chartType == 'metrix'){
		series_arr[z].objectMap[arcObject.ix+'x'+arcObject.iy] = arcObject;	
	}else{
		series_arr[z].objectMap[t_lx_label] = arcObject;
	}


	if(this.zoom == 1){
		if(typeof series_arr[z].objectMapOri == 'undefined'){
			//series_arr[z].objectArrOri = new Array();
			series_arr[z].objectMapOri = new Array();
		}
		//series_arr[z].objectArrOri.push(arcObject);
		series_arr[z].objectMapOri[t_i] = arcObject;
	}					

	this.bufferCtx.fillStyle = series_arr[z].s_color;
	this.bufferCtx.strokeStyle = series_arr[z].s_borderColor;
	if(series_arr[z].s_chartType == 'metrix'){
	...
	if(series_arr[z].s_chartType == 'bar'){
		...
		var _y = t_y;
		this.bufferCtx.fillRect(_x, _y,(xAxisObj.tikGepLength-2),this.centerCanvas.height-t_y );
	}else if(series_arr[z].s_chartType == 'metrix'){	
		var d = this.toPhysicalValSize(yAxisObj.div);
		this.bufferCtx.fillRect(t_x, t_y-d,xAxisObj.tikGepLength,d );
	} else {
	}
...
MbaseChart.prototype.drawXAxis_spline_drawX = function(centerObj,xAxisObj,yAxisObj,series_arr,isDrawXAxis,arr,nowIndex,dotted_gep,excep_str,_maxSeriesIndex) {
	if(this.half_x_axis && nowIndex > this.left_indexs-1){
    	dotted_gep = 1;
    }
	...
	var divid_rest;
	if(this.type== 'metrix'){
		nowIndex = nowIndex+xAxisObj.min;
		divid_rest = 15;
	}else{
		divid_rest = 0;
	}
	...
	if(nowIndex == 0 || nowIndex % aGep == divid_rest || excep_str == 'end' || (this.type == 'metrix') ){
		...
		if(this.type == 'metrix'){
			//x좌표
			//xTikLabel = (nowIndex+1)*(this.x_max_re/((this._x_max-this._x_min)+1));
			xTikLabel = nowIndex*(this.x_max_re-this.x_min_re)/(this._x_max-this._x_min);
			var dv = (this.x_max_re-this.x_min_re)/this.x_div_cnt;
			if(xTikLabel%dv != 0){
				xTikLabel = "";
			}
/*			if(xTikLabel%10 != 0){
		...
		var metrics = xAxisObj.bottomCtx.measureText(xTikLabel);
		var mwidth = metrics.width;
		
		if(this.half_x_axis && (seq > this.left_indexs-1)){
	    	x_now = this.getXByHalf(seq);
	    }else{
	    	//x축 좌표계 그리기
	    	if(this.type == 'metrix'){
	    		x_now = xAxisObj.tikGepLength*seq+xAxisObj.tikGepLength/2;
	    	}else{
		...
		if(excep_str == 'start'){
			...
			if(this.type == 'metrix'){
				...
				if(this.xangle){
					...
					xAxisObj.bottomCtx.rotate(-this.xangle*Math.PI/180);
...
MbaseChart.prototype.getXByHalf = function(_seq){
	return (this.xAxisObj.tikGepLength*(this.left_indexs-1))+(this.xAxisObj.tikGepLength2*(_seq-(this.left_indexs-1)));
}
//보정라인굵기값 지정시작
...
MbaseChart.prototype.drawXAxis_spline2 = function(centerObj,xAxisObj,yAxisObj,series_arr,isDrawXAxis) {
	...
	if(isDrawXAxis){
		...
		//centerObj.centerCtx.drawImage(this.bottomCanvas,this.ly_width,this.centerCanvas.height);
	}	
...
MbaseChart.prototype.drawXAxis_spline_cat = function(centerObj,xAxisObj,yAxisObj,series_arr,isDrawXAxis) {
	...
	//var gridFlag = true;
	if(this.x_terms != null){
		for(var i=0; i < this.x_terms.length; i++){
			var addInfoObj = this.x_terms[i];
			if(addInfoObj.s_draw){
				...
				this.bufferCtx.fillStyle = addInfoObj.termColor;
				this.bufferCtx.fillRect(f,0,t-f,this.centerCanvas.height);
				this.bufferCtx.fill();
...
/*		series_arr[z].objectArr = new Array();
		series_arr[z].objectArrOri = new Array(); */
		series_arr[z].objectMap = {};
		series_arr[z].objectMapOri = {};		
...
/*			this.s_ctx[z].beginPath();
			...
			this.bufferCtx.beginPath();
			this.bufferCtx.fillStyle = series_arr[z].s_color;
			this.bufferCtx.strokeStyle = series_arr[z].s_color;
		}
		...
		for(var i=startIndex; i < loopCnt; i++){
			...
			if(this.half_x_axis && (i > (this.left_indexs-1))){
				tik_x = this.getXByHalf(i);
		    	//offset2
		    }else{
...
/*				if(
				...
				if(!spline_end && stype == 'spline'){
					var hcon = this.half_x_axis && i > this.left_indexs-1;
					...
					if(i == 0){
						...
						if(typeof _dataArr2[i+1] != 'undefined' && typeof _dataArr2[i+1][z] != 'undefined'){
						...
						if(typeof _dataArr2[i+2] != 'undefined' && typeof _dataArr2[i+2][z] != 'undefined'){
...
/*						if(z == series_arr.length-1){
							spline_end = true;
							console.log(z+'/end proc/'+series_arr.length);
						}*/
					}else if(i == _dataArr2.length -1){
						if(typeof _dataArr2[i][z] != 'undefined'){
							p_1 = [(hcon)? this.getXByHalf(i):v*i,(series_arr[z].s_axis == 'right')? this.toPhysicalVal2(_dataArr2[i][z][1]):this.toPhysicalVal(_dataArr2[i][z][1])];
					...
					if(p_0 != null && p_1 != null && p_2 != null && p_3 != null){
						...
						if(i == this.left_indexs){
							this.CatmullRomSplines(this.bufferCtx,{_x:p_0[0],_y:p_0[1]},{_x:p_0[0],_y:p_0[1]},{_x:p_1[0],_y:p_1[1]},{_x:p_2[0],_y:p_2[1]});
							this.CatmullRomSplines(this.bufferCtx,{_x:p_0[0],_y:p_0[1]},{_x:p_1[0],_y:p_1[1]},{_x:p_2[0],_y:p_2[1]},{_x:p_3[0],_y:p_3[1]});
						}else if(i == this.left_indexs-1){
							//this.CatmullRomSplines(this.bufferCtx,{_x:p_0[0],_y:p_0[1]},{_x:p_0[0],_y:p_0[1]},{_x:p_1[0],_y:p_1[1]},{_x:p_2[0],_y:p_2[1]});
						}else{
							this.CatmullRomSplines(this.bufferCtx,{_x:p_0[0],_y:p_0[1]},{_x:p_1[0],_y:p_1[1]},{_x:p_2[0],_y:p_2[1]},{_x:p_3[0],_y:p_3[1]});
						}
					...
					if(typeof _dataArr2[i][z] != 'undefined'){
						...
						var p_x_pre;
						if(this.half_x_axis && (i-1) > this.left_indexs-1){
					    	p_x_pre = this.getXByHalf(i-1);
					    }else{
					    	p_x_pre = (i-1)*xAxisObj.tikGepLength;
					    }								
						this.bufferCtx.moveTo(p_x,p_y);
						this.bufferCtx.lineTo(p_x_pre,p_y_pre);
					}
				}
				
				//case of independent series, and metrix draw!
				if(typeof _dataArr2[i][z] != 'undefined'){
		...
		}//for	for(var i=startIndex; i < loopCnt; i++){ 
	...
	if(this.arrow){
	    ...
	    for(var z=this.sortArr.length-1; z > -1; z--){
			...
			//if(x < _dataArr2.length){
				...
				var x_phsic = (x-xAxisObj.min)*xAxisObj.tikGepLength;
				if(z == 0){
					this.drawArrow(x_phsic,t_y,degree,arrow_width,arrow_height,this.arrow_no1_color);
					this.showInfo(this.sortArr[z],x_phsic,t_y);
				}else{
					degree = this.getArrowPosition(t_x,t_y,arrow_width,arrow_height,5);
					if(this.arrow_colors != null && this.arrowColorsObj.length+1 > z){
						this.drawArrow(x_phsic,t_y,degree,arrow_width,arrow_height,this.arrowColorsObj[z-1]);
					}else{
						this.drawArrow(x_phsic,t_y,degree,arrow_width,arrow_height,this.default_arrow_color);
					}
				}
...
//best화살표 표시
MbaseChart.prototype.getArrowPosition = function(lastX,lastY,tooltipRectX,tooltip_rect_y,toolTipMargin) {
	var idex = 0;
...
MbaseChart.prototype.getDottedGep = function(_dataArr2,_text) {
    ...
    if(_dataArr2.length > maxTikCnt){
        if(this.half_x_axis){
        	dotted_gep = this.left_indexs/maxTikCnt;
        }else{
...
MbaseChart.prototype.init = function(options) {
	...
	if(typeof options['chartId'] == 'undefined'){
		console.dir('options에 chartId 가 없습니다!');
	}
	...
	if(this.arrow_colors != null && this.arrow_colors != ''){
		this.arrowColorsObj = JSON.parse(this.arrow_colors);	
	}
	this.menuBoxWidth = 20;
	...
	//options.type = 'metrix'; 없어도 metrix적용되도록 변경
	for(var k=0; k < this.series.length; k++){
		var seriesObj = this.series[k];
		if(seriesObj.s_chartType == 'metrix'){
			this.type = 'metrix';
	...
	this.y_base_lines = options.y_base_lines;
	this.x_terms = options.x_terms;
	...
	this.yAxis_x = this.left_margin;
	this.yAxis_yStart = this.canvasHeight;
	this.yAxis_yEnd = this.top_margin;


	

	this.xAxis_xEnd = this.centerCanvasWidth-this.right_margin;
...
//툴팁표시위치
MbaseChart.prototype.getToolTipPosition = function(lastX,lastY,tooltipRectX,tooltip_rect_y,toolTipMargin) {
	var obj = {};
	var idex = 1;
			if(lastX < this.canvas.width/2){
				if(lastY < this.canvas.height/2){
					...
					idex = 3;
					rect_start_x = lastX;rect_start_y = lastY-tooltip_rect_y;
				...
				if(lastY < this.canvas.height/2){
					...
					idex = 4;					
					rect_start_x = lastX-tooltipRectX;rect_start_y = lastY-tooltip_rect_y;
					rect_start_x -= toolTipMargin;
					rect_start_y -= toolTipMargin;
				}	
			}
...
MbaseChart.prototype.saveParams = function() {
	...
	this.saveObj.data_arr = this.data_arr.slice(0);
	this.saveObj._dataArr_ori = this._dataArr_ori.slice(0);
	this.saveObj.axes_font_size = this.axes_font_size;
	this.saveObj.left_margin = this.left_margin;
	this.saveObj.right_margin = this.right_margin;
	this.saveObj.top_margin = this.top_margin;
...
MbaseChart.prototype.draw = function(__dataArr) {
	if(__dataArr != null){		
		this.data_arr = __dataArr;
		this._dataArr_ori = this.data_arr.slice(0);
	}
...
//그릴때 항상 호출되는 마무리용 그리기
MbaseChart.prototype.callbackDraw = function() {
/*	this.fullCtx.drawImage(this.leftCanvas,0, 0);
	this.fullCtx.drawImage(this.rightCanvas,this.leftCanvas.width+this.centerCanvas.width, 0);
...
/*	this.fullCtx.fillStyle = 'black';
	...
	if(this.use_right_axis_only || this.axis_use_cnt == 2){
		this.ctx.drawImage(this.rightCanvas,this.leftCanvas.width+this.centerCanvas.width, 0);
	}
	this.ctx.drawImage(this.bufferImage,this.leftCanvas.width, 0); 
	this.ctx.drawImage(this.bottomCanvas,this.leftCanvas.width, this.centerCanvas.height);
...
MbaseChart.prototype.drawCharts = function() {
	...
	if(this.use_right_axis_only){
		this.drawYAxis_common(this.centerObj,this.xAxisObj,this.yAxisObj2,series_arr);
		this.drawXAxis_spline(this.centerObj,this.xAxisObj,this.yAxisObj2,series_arr,this.drawXAxis);
	}else{
		this.drawYAxis_common(this.centerObj,this.xAxisObj,this.yAxisObj,series_arr);
		
		var leftIndex = -1;
		var rightIndex = -1;
		this.type = series_arr[0].s_chartType;
...
function procK(n){
	...
	if(Math.abs(n/1000) > 1 || Math.abs(n/1000) == 1){
		return (parseFloat(parseInt(n/1000)+'.'+parseInt(n%1000))).toFixed(2)+'k';
	}else{
		var tmp = n.toFixed(2);
		if((tmp+'').match(/.00$/)){
			return n;
...
function setSize(mobj,w,h,key){
		var lHeight;
		if(sizes_w[key] == null){
			var minMaxObj = mobj.getMinMax(series_array[key]);
			var c = mobj.canvas.getContext("2d");
			
		
					
			var mwidth_min = c.measureText(""+comma(procK(minMaxObj.min))).width;
			var mwidth_max = c.measureText(""+comma(procK(minMaxObj.max))).width;
	
			if(mwidth_min > mwidth_max){
				mwidth = mwidth_min;
			}else{
				mwidth = mwidth_max;
			}
			...
			lHeight = w*sizes_w[key];
		}
	
		var b_height = h*sizes_h[key];
		var rHeight = w*sizes_w2[key];
		$(mobj.canvas).attr('width',w);$(mobj.canvas).attr('height',h);
	    $(mobj.leftCanvas).attr('width',lHeight );$(mobj.leftCanvas).attr('height',h-b_height );
	    $(mobj.rightCanvas).attr('width',rHeight );$(mobj.rightCanvas).attr('height',h-b_height );
	    $(mobj.bottomCanvas).attr('width',w-lHeight-rHeight );$(mobj.bottomCanvas).attr('height',b_height );
...
function starts(mobj,chartName,option){
	if(chartName == null){
		...
		if(option == null){
		    ...
		    sizes_w[key] = option.ly_width;
		    sizes_w2[key] = option.ry_width;
		    sizes_h[key] = option.b_height;
			var container = mobj.canvas.parentNode; 
			mobj.container = container;
			var w = $(container).width();
			var h = $(container).height();
...
//체크박스 체크나 해제시 동작
function _chart_inner_show(_chkObj,_s_name,_id){
	for(var i=0;i < charts[_id].series.length; i++){
		var sa = charts[_id].series[i];
		if(sa.s_name == _s_name){
			if(_chkObj.checked){
				sa.s_draw = true;
			}else{
				sa.s_draw = false;	
...
//x축 변화율 체크박스 선택및 해제시 동작
function _chart_inner_show_x(_chkObj,_s_name,_id){
	for(var i=0;i < charts[_id].x_terms.length; i++){
		var sa = charts[_id].x_terms[i];
		if(sa.termLabel == _s_name){
			if(_chkObj.checked){
				sa.s_draw = true;
			}else{
				sa.s_draw = false;	
...
//y축 기준선  선택및 해제시 동작
function _chart_inner_show_y(_chkObj,_s_name,_id){
	for(var i=0;i < charts[_id].y_base_lines.length; i++){
		var sa = charts[_id].y_base_lines[i];
		if(sa.alramLabel == _s_name){
			if(_chkObj.checked){
				sa.s_draw = true;
			}else{
				sa.s_draw = false;	
...
//옵션 적용하여 다시 그리기
function _chart_option_update(_id){
	var arr = $('input[id="_chart_optionsInput_'+_id+'"]');
	for(var i=0; i < arr.length; i++){
		var a = arr[i];
		var o = charts[_id][a.name];
		if(typeof o == 'string' || o == null){//중요한 옵션임 null은 object으로 인지함!!!
			
			charts[_id][a.name] = a.value;
			
			if(a.name == 'arrow_colors'){
				if(a.value != null && a.value !=''){
					try{
						charts[_id]['arrowColorsObj'] = JSON.parse(charts[_id][a.name]);
					}catch(e){
						console.dir(e+'error : this.arrow_colors:'+charts[_id][a.name]);	
					}						
				}else{
					charts[_id]['arrow_colors'] = null;
					charts[_id]['arrowColorsObj'] = null;
				}
			}
			...
			charts[_id][a.name] = a.value*1;
		}else if(typeof o == 'boolean'){
			charts[_id][a.name] = (a.value=="true") ? true : false;
		}else{

		}
		charts[_id].options[a.name] = charts[_id][a.name];
	}
	redraw(_id);
}

function redrawAll(_id,_opts){
	$.each( _opts, function( key, value ) {
		if(typeof value != 'object'){
			var o = charts[_id][key];
			if(typeof o == 'string'){
				charts[_id][key] = value;
			}else if(typeof o == 'number'){
				charts[_id][key] = value*1;
			}else if(typeof o == 'boolean'){
				charts[_id][key] = (value=="true" || value==true) ? true : false;
			}

			charts[_id].options[key] = charts[_id][key];
		}
	});
	redraw(_id);
}
...
//옵션 초기화
function _chart_option_reset(_id){
	var str = localStorage[_id];
	var opts = JSON.parse(str);
	var arr = $('input[id="_chart_optionsInput_'+_id+'"]');
	for(var i=0; i < arr.length; i++){
		var a = arr[i];
		a.value = opts[a.name];
		if(/.*Color$/.test(a.name) || /.*color$/.test(a.name)){
			a.closest("td").style.backgroundColor = a.value;
		}
	}
}
...
function setChart(a,divObj,optionx){
	...
	var legendTable = '<table class="tb" style="height:0" id="_____legendTable_'+optionx.chartId+'"><thead><tr><th class="tb01"></th><th class="tb02">시리즈</th><th class="tb02">색상</th></tr></thead><tbody>';
	var series_array = optionx.series;
	for(var i=0;i < series_array.length; i++){
		var sa = series_array[i];
		legendTable += '<tr><td class="tb03_b" style="width:20px"><input type="checkbox" class="checkbox" checked  onclick="_chart_inner_show(this,\''+sa.s_name+'\','+optionx.chartId+')"></td>';
		legendTable += '<td class="tb03" style="text-align:left">'+sa.s_name+'</td>';
		legendTable += '<td class="tb03"  name="_chart_colorTd1" s_name="'+sa.s_name+'" style=\'background-color:'+sa.s_color+';width:40px\'></td></tr>';
	}
	...
	var xTable = '<table class="tb" style="height:0" id="xTable_'+optionx.chartId+'"><thead><tr><th class="tb01"></th><th class="tb02">X구간</th><th class="tb02">색상</th></tr></thead><tbody>';
	var xTerms_array = optionx.x_terms;
	for(var i=0;i < xTerms_array.length; i++){
		var sa = xTerms_array[i];
		xTable += '<tr><td class="tb03_b" style="width:20px"><input type="checkbox" class="checkbox"   onclick="_chart_inner_show_x(this,\''+sa.termLabel+'\','+optionx.chartId+')"></td>';
		xTable += '<td class="tb03" style="text-align:left">'+sa.termLabel+'</td>';
		xTable += '<td class="tb03"  name="colorTd2" s_name="'+sa.termLabel+'" style=\'background-color:'+sa.termColor+';width:40px\'></td></tr>';
	}
	xTable += '</tbody></table>';
	
	var yTable = '<table class="tb" style="height:0" id="yTable_'+optionx.chartId+'"><thead><tr><th class="tb01"></th><th class="tb02">Y기준선</th><th class="tb02">색상</th></tr></thead><tbody>';
	var yBaseLines_array = optionx.y_base_lines;
	for(var i=0;i < yBaseLines_array.length; i++){
		var sa = yBaseLines_array[i];
		yTable += '<tr><td class="tb03_b" style="width:20px"><input type="checkbox" class="checkbox"   onclick="_chart_inner_show_y(this,\''+sa.alramLabel+'\','+optionx.chartId+')"></td>';
		yTable += '<td class="tb03" style="text-align:left">'+sa.alramLabel+'</td>';
		yTable += '<td class="tb03"  name="colorTd3" s_name="'+sa.alramLabel+'" style=\'background-color:'+sa.alramColor+';width:40px\'></td></tr>';
	}
	yTable += '</tbody></table>';
	
	var optionTable = '<table class="tb" style="height:0;margin-top:5px" id="optionTable_'+optionx.chartId+'"><thead><tr><th class="tb02">속성</th><th class="tb02">값</th></tr></thead><tbody>';
	var storageData = {};
	
	var keys = [];	
	for (k in optionx)
	{
	    if (optionx.hasOwnProperty(k))
	    {
	        keys.push(k);
	    }
	}
	...
	for (i = 0; i < len; i++)
	{
	    key = keys[i];
	    value = optionx[key];
		if(typeof value != 'object' && key != 'zoom' && key != 'chartId'){
			optionTable += '<tr><td style="vertical-align:middle; color: #fff;text-align:left"><a title="aaa" style="color:#000000;">'+key+'</a></td>';
			if(/.*Color$/.test(key) || /.*color$/.test(key)){
				optionTable += '<td style="vertical-align:middle; color: #fff;background-color:'+value+'"  name="'+key+'" ><input type=text id="_chart_optionsInput_'+optionx.chartId+'" name="'+key+'" value=\''+value+'\' style="width:100%;display:none"></td></tr>';
			}else{
				optionTable += '<td style="vertical-align:middle; color: #fff;"   ><input type=text id="_chart_optionsInput_'+optionx.chartId+'" name="'+key+'" value=\''+value+'\' style="width:100%;"></td></tr>';				
			}
  	...
  	if(typeof localStorage !== 'undefined'){
  		localStorage[optionx.chartId] = JSON.stringify(storageData);
  	}
	
	optionTable += '</tbody></table>';	
	
	var buttons = "<input type='button' class='button' style='width:50%;height:10%' value='적용' onclick=\"_chart_option_update('"+optionx.chartId+"')\"></input>";
	buttons += "<input type='button' class='button' style='width:50%;height:10%' value='초기화' onclick=\"_chart_option_reset('"+optionx.chartId+"')\"></input>";
	
	divObj.html("<div id='chartLayer'  style='width:100%;height:100%;float:left;margin: 0 auto;overflow:hidden'></div><div id='optLayer' show=false style='width:0%;height:100%;float:right;overflow:hidden'><div style='width:100%;height:90%;float:right;overflow-y:scroll'>"+legendTable+xTable+yTable+optionTable+"</div><div style='width:100%;height:100%;'>"+buttons+"</div></div>");
	$(divObj[0].childNodes[0]).append(b);
	
	
	
	var hideit_option = function(e, ui) { 
				$(this).css('background-color', ('#'+ui.hex) );
				$(this).children("input").val('rgba('+ui.rgb.r+','+ui.rgb.g+','+ui.rgb.b+',1)');
			    $('.ui-colorpicker').css('display', 'none'); 
    };
	$("td[name$=\"Color\"]").colorpicker({ hide: hideit_option, submit: hideit_option });
	$("td[name$=\"color\"]").colorpicker({ hide: hideit_option, submit: hideit_option });
	var hideit1 = function(e, ui) { 
				$(this).css('background-color', ('#'+ui.hex) );
			    var objectName = this.getAttribute('s_name');
				for(var i=0;i < series_array.length; i++){
					var sa = series_array[i];
				  	if(sa.s_name == objectName){
				  		sa.s_color = 'rgba('+ui.rgb.r+','+ui.rgb.g+','+ui.rgb.b+',1)';
				  		redraw(optionx.chartId);
						break;  
				  	}
			    }
			    $('.ui-colorpicker').css('display', 'none'); 
    };
    $("td[name=\"_chart_colorTd1\"]").colorpicker({ hide: hideit1, submit: hideit1 });
    
    var hideit2 = function(e, ui) { 
				$(this).css('background-color', ('#'+ui.hex) );
			    var objectName = this.getAttribute('s_name');
				for(var i=0;i < xTerms_array.length; i++){
					var sa = xTerms_array[i];
				  	if(sa.termLabel == objectName){
				  		var str = sa.termColor;
				  		var str_arr = str.split(',');
				  		var a = str_arr[str_arr.length-1].replace(')','');
				  		sa.termColor = 'rgba('+ui.rgb.r+','+ui.rgb.g+','+ui.rgb.b+','+a+')';
				  		redraw(optionx.chartId);
						break;  
				  	}
			    }
    ...
    $("td[name=\"colorTd2\"]").colorpicker({ hide: hideit2, submit: hideit2 });
    
    var hideit3 = function(e, ui) { 
				$(this).css('background-color', ('#'+ui.hex) );
			    var objectName = this.getAttribute('s_name');
				for(var i=0;i < yBaseLines_array.length; i++){
					var sa = yBaseLines_array[i];
				  	if(sa.alramLabel == objectName){
				  		sa.alramColor = 'rgba('+ui.rgb.r+','+ui.rgb.g+','+ui.rgb.b+',1)';
				  		redraw(optionx.chartId);
						break;  
				  	}
			    }
    ...
    $("td[name=\"colorTd3\"]").colorpicker({ hide: hideit3, submit: hideit3 });
...
MbaseChart.prototype.metrix_reval_y = function(_tmp_v) {
	var tmp_v = _tmp_v*((this.y_max_re-this.y_min_re)/((this._y_max-this._y_min)))+this.y_min_re;
	return tmp_v;
}

MbaseChart.prototype.metrix_reval_x = function(_tmp_v) {
	var tmp_v = _tmp_v*((this.x_max_re-this.x_min_re)/((this._x_max-this._x_min)))+this.x_min_re;
	return tmp_v;
}
