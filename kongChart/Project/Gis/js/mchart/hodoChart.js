...
HChart.prototype.drawCrossLines = function(M,_event) {
	...
	if(M.o()['pivot_mode'] == 'XY'){
		...
		if(M.getLeftAxisWidth() < lastX && lastX < (M.getLeftAxisWidth()+M.getBottomAxisWidth())){
			if(M.o()['top_margin'] < lastY && lastY < (M.o()['top_margin']+M.getLeftAxisHeight())){
				var coord = {'x':'','y':''};
					coord['y'] = M.toLogicalVal_L_Y(lastY+M.getMarkGepLeft()/2,M.o()['series'][M.o()['minmax']['xCntMaxSIndex']]['s_min'],M.o()['series'][M.o()['minmax']['xCntMaxSIndex']]['s_max']).toFixed(2);	
					coord['x'] = M.toLogicalVal_B_X(ex+M.getMarkGepBottom()/2).toFixed(2);
					var wd = M.conv_uv2wd(parseFloat(coord['x']),parseFloat(coord['y']));
					var ws = M.conv_uv2ws(parseFloat(coord['x']),parseFloat(coord['y']));
					coord_text = '        [ '+coord['x']+'/u , '+coord['y']+'/v , '+wd.toFixed(2)+'/wd , '+ws.toFixed(2)+'/ws ]';
...
/*
				for(var i=0;i< this.o()['series'].length; i++){
					...
					for(var j=0;j < s_data.length; j++){
						...
						var y = parseFloat(s_data[j][5])+5;//v y축
/*						x = 0-5;
						y = 0+5;*/
						var px = this.toPhysicalVal_B_X(x);
						var py = this.toPhysicalVal_L_Y(y,this.o()['series'][i]['s_min'],this.o()['series'][i]['s_max']);
...
HChart.prototype.drawXAxis = function() {
	...
	*/
	...
	if(pmode == 'XY'){
		...
		//4각도표시----------------------------------------------북,남
		this.mainCanvasBufferCtx.fillStyle = this.o()['color'];
		var f = this.mainCanvasBufferCtx.font;
		this.mainCanvasBufferCtx.font = 'bold 20px Arial';
		this.mainCanvasBufferCtx.fillText('270o',
				this.f(startx_axis)-50,this.f(this.o()['top_margin']+this.getLeftAxisHeight()/2)+8			
		);
		this.mainCanvasBufferCtx.fillText('90o',
				this.f(startx_axis+this.getBottomAxisWidth())+2,this.f(this.o()['top_margin']+this.getLeftAxisHeight()/2)+8
		);		
		this.mainCanvasBufferCtx.font = f;
		var axisLeng = 1;
...
HChart.prototype.drawYAxis = function() {
	...
	*/
	...
	if(pmode == 'XY'){
		...
		var f = this.mainCanvasBufferCtx.font;
		this.mainCanvasBufferCtx.font = 'bold 20px Arial';
		this.mainCanvasBufferCtx.fillText('360o',
				this.f(_x1+_width1/2)-20,
				this.f(_y1)-2			
		);
		this.mainCanvasBufferCtx.fillText('180o',
				this.f(_x1+_width1/2)-20,this.f(_y1+_height1)+20	
		);		
		this.mainCanvasBufferCtx.font = f;
		var axisLeng = 1;
		if(this.o()['multiYAxis']){
...
HChart.prototype.drawWindflag = function(x,y,_degree,width,height,_color,windspeed,_flag) {
    ...
    this.mainCanvasBufferCtx.rotate(_degree*Math.PI/180);
		this.mainCanvasBufferCtx.beginPath();
			...
			this.mainCanvasBufferCtx.strokeStyle = _color;
			this.mainCanvasBufferCtx.moveTo(0, 0);
			this.mainCanvasBufferCtx.lineTo(0, 0 + height);
			this.mainCanvasBufferCtx.stroke();	
			var restVal = 0;
			if(parseInt(windspeed/v3) == 1){
				this.mainCanvasBufferCtx.lineTo(0,base_height);
				this.mainCanvasBufferCtx.lineTo(-(0 + width), base_height - flag_gep+subLeng);
				this.mainCanvasBufferCtx.lineTo(0, base_height - flag_gep*2);
				this.mainCanvasBufferCtx.fill();
			...
			var display = false;
			for(var i=(fval*2),j=0; i < restVal; i+=(fval*2),j++){
				//if(j%2 == 1 ){
					//this.mainCanvasBufferCtx.fillStyle = 'rgba(255,0,0,1)';
					//this.mainCanvasBufferCtx.strokeStyle = 'rgba(255,0,0,1)';
					this.mainCanvasBufferCtx.moveTo(0, base_height);
					this.mainCanvasBufferCtx.lineTo(-(0 + width), base_height+subLeng);
					this.mainCanvasBufferCtx.stroke();
					base_height -= flag_gep;					
				//}else if(j%2 == 0 ){
					//this.mainCanvasBufferCtx.fillStyle = 'rgba(255,0,0,1)';
					//this.mainCanvasBufferCtx.strokeStyle = 'rgba(255,0,0,1)';
			...
			if(display){
				this.mainCanvasBufferCtx.moveTo(0, base_height);
				this.mainCanvasBufferCtx.lineTo(-(0 + width/2), base_height+subLeng/2);
				this.mainCanvasBufferCtx.stroke();		
			}
	...
	/*		
			if(windspeed < 2 || windspeed == 2){
				...
				this.mainCanvasBufferCtx.fillStyle = _color;
				this.mainCanvasBufferCtx.strokeStyle = 'rgba(0,0,0,1)';
				this.mainCanvasBufferCtx.moveTo(0, 0);
				this.mainCanvasBufferCtx.lineTo(0, 0 + height);
				this.mainCanvasBufferCtx.stroke();
				...
				this.mainCanvasBufferCtx.fillStyle = 'rgba(255,0,0,1)';
				this.mainCanvasBufferCtx.strokeStyle = 'rgba(255,0,0,1)';
				this.mainCanvasBufferCtx.moveTo(0, base_height-5);
				this.mainCanvasBufferCtx.lineTo(-(0 + width/2), base_height);
				this.mainCanvasBufferCtx.stroke();
				
				base_height -= flag_gep;
			}else if(windspeed < 12.1){
				this.mainCanvasBufferCtx.fillStyle = _color;
				this.mainCanvasBufferCtx.strokeStyle = 'rgba(0,0,0,1)';
				this.mainCanvasBufferCtx.moveTo(0, 0);
				this.mainCanvasBufferCtx.lineTo(0, 0 + height);
				...
				this.mainCanvasBufferCtx.strokeStyle = 'rgba(255,0,0,1)';
				this.mainCanvasBufferCtx.moveTo(0, base_height);
				this.mainCanvasBufferCtx.lineTo(-(0 + width), base_height+5);
				this.mainCanvasBufferCtx.stroke();
				
				base_height -= flag_gep;
			}else if(windspeed > 12 && windspeed < 17.1){
				this.mainCanvasBufferCtx.fillStyle = _color;
				this.mainCanvasBufferCtx.strokeStyle = 'rgba(0,0,0,1)';
				this.mainCanvasBufferCtx.moveTo(0, 0);
				this.mainCanvasBufferCtx.lineTo(0, 0 + height);
				...
				this.mainCanvasBufferCtx.fillStyle = 'rgba(255,0,0,1)';
				this.mainCanvasBufferCtx.strokeStyle = 'rgba(255,0,0,1)';
				this.mainCanvasBufferCtx.moveTo(0, base_height-2.5);
				this.mainCanvasBufferCtx.lineTo(-(0 + width/2), base_height);
				this.mainCanvasBufferCtx.stroke();
				base_height -= flag_gep;
			}else if(windspeed > 17 && windspeed < 22.1){
				this.mainCanvasBufferCtx.fillStyle = _color;
				this.mainCanvasBufferCtx.strokeStyle = 'rgba(0,0,0,1)';
				this.mainCanvasBufferCtx.moveTo(0, 0);
				this.mainCanvasBufferCtx.lineTo(0, 0 + height);
...
HChart.prototype.calcDate = function(reDate,_hgep) {
 ...
 HChart.prototype.conv_uv2ws = function(_u,_v) {
	var d = Math.sqrt(_u*_u+_v*_v);
    return d;

}
 
 
HChart.prototype.conv_uv2wd = function(_u,_v) {
	var pi, r2d;
    var wd;
    if (_u == 0 && _v >= 0)
        return 0;
    ...
    r2d = 180. / pi;
    if (_u > 0)
        wd = 90 - Math.atan(_v / _u) * r2d;
    else
        wd = 270 - Math.atan(_v / _u) * r2d;
