...
HChart.prototype.drawGradient = function(_option) {
	...
	for(var v=zmin,i=0 ; v < zmax ; v=v+div,i++){
		...
		if(parseInt(i) % 4 == 0){
				...
				this.mainCanvasBufferCtx.fillText((zmin+(div*(i+1))).toFixed(this.o()['toFixedY']),
...
/*				for(var i=0;i< this.o()['series'].length; i++){
						...
						if(y != null && typeof y != 'undefined'){
							...
							if(!sLabelPrintYn){
									if(typeof this.o()['series'][i]['unitLabel_Hori'] != 'undefined' && lastExistY != ''){
										...
										console.log(lastExistY+'----->'+this.o()['series'][i]['unitLabel_Hori']);
										sLabelPrintYn = true;		
									}	
							}
				...
				/*}*/
					...
					var prePonit = {};
					for(var j=0;j < s_data.length; j++){
						var x = s_data[j][0];//-몇시간
						var y = s_data[j][3];//고도
						var y1 = s_data[j][4];//풍향
						var y2 = s_data[j][5];//풍속
						
						var label = s_data[j][2];//X좌표값라벨
						...
						if(typeof y1 != 'undefined' && y1 != null  ){
							...
							if(s_shape == 'arrow'){
								...
								if(typeof y1 != 'undefined'){
									this.drawArrow(px,py,y1,5,8,colorText);//화살표의키,화살표의폭
								}
							}else if(s_shape == 'windflag'){
									//var color = this.o()['colorObj'].getColour(y2);
									var colorText = 'rgba(62,52,192,1)';
									if(typeof y2 != 'undefined'){
										this.drawWindflag(px,py,y1,8,20,colorText,y2,this.o()['windflag_unit']);//y2:풍속
									}
	...
	if(pmode == 'XY'){
		...
		for(var i=startx; i < endx+1; i+=markGep) {
			...
			this.mainCanvasBufferCtx.fillText(x_label,
					...
					this.f(y+this.o()['mark_width']+this.o()['mark_then_gep']+this.o()['leftLabelMaxHeight'])
					);
