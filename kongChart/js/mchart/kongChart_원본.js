...
HChart = function(_container) {
	...
	this.defaultColours = [
		                ...
		                ];//占쎈쵐占쏙옙�ъ쁽 獄쏆빓苡�占쎄쑴類꾬옙占�占쎄퀣�쏙옙占쏙옙��ぜ野껓옙占쎄쑬�믦����
	...
	this.defaultOption = {
		...
		mark_then_gep:2,
		gep_space:2,
		//x_mark_cnt:11,//雅뚯눘堉깍쭪袁⑸떭占쎈Ŧ�ワ옙占�占쎈쵐占�mark揶쏉옙占쎌빓占�-2(占쏙옙占쎌꼵而쇿칰占썽뜮�⑦� 0甕곕뜆�륅옙占�1占쎌꼶占썸에占�/
		x_mark_cnt:10,
		y_mark_cnt:10,
		min:-100,
		max:100,
						...
						$.each(point, function(key, obj) { 
							...
							if(typeof obj != 'undefined' && obj != null){
								...
								if(obj['value'] != null){
									...
									if(M.o()['xrange_axis']){
										coord_text = '['+obj['logical']+':'+obj['physical']+' '+obj['value']+']';
									}else{
										coord_text = '['+obj['name']+':'+M.comma(obj['value'])+']';
	...
	if(startx_axis < _ex < endx_axis){
		//for(var i=0;i< this.o()['series'].length; i++){
			...
			if(xval_range_start < _ex 
				...
				/*
				var x = s_data[j][0];
				reObj['label'] 		= s_data[j][0]+':'+s_data[j][2];
				reObj['logical'] 	= xval;
				reObj['physical'] 	= xval;
				reObj['value'] 		= s_data[j][1];
				return reObj;
				*/
...
HChart.prototype.init = function(_option) {
	...
	//this.o()['leftLabelMaxWidthXY']  		= this.mainCanvasBufferCtx.measureText(this.o()['minmax']['max']).width+this.o()['leftLabelMaxWidthXYAddGep'];
...
//////////////////////////////방사형옵션 start////////////////////////	
	...
	//ym = this.getLeftAxisHeight()/2;
	ym = this.getLeftAxisHeight()/2-25;
	if(xm > ym){
		this.o()['radius'] = ym;		
	}else{
		//this.o()['radius'] = xm-100;			
		this.o()['radius'] = ym;	
	}
...
HChart.prototype.redraw = function(_option,_param1) {
	...
	//this.mainCanvasCtx.drawImage(this.mainCanvasBuffer,0,0);
	*/
...
HChart.prototype.drawBottomTitle = function(_option) {
	/*
	this.mainCanvasBufferCtx.font = this.o()['font'];
	this.mainCanvasBufferCtx.fillStyle = this.o()['color'];
	this.mainCanvasBufferCtx.fillText(this.o()['chartTitle'],
			...
			this.mainCanvas.height-this.o()['mark_then_gep']
			);
	*/			
	try{
		var legendWidth = 100;
...
HChart.prototype.drawLegend = function(
	...
	if(
		...
		//console.dir('s_chartType:'+s_chartType+':'+s_shape);
		this.drawArc4(
				s_color,
				s_shapeColor,
				s_lineWidth,
	...
	this.fillRect(
			...
			s_legendWidth+gep+2,
			_y1+gep+2,
			this.o()['legendItemBoxWidth']+legendWidth+4,
			12+4,
			this.mainCanvasBufferCtx
			);
	*/			
						...
						if(y != null && typeof y != 'undefined'){
							...
							if(this.o()['dotShow']){
								...
								if(this.o()['x_label_extension']){
									this.drawTipLine (s_color,45,_x1,_y1,_width1,30,xlabel);
								}
							startx_axis = px; 
							starty = py;	
						}else{
...
/*				for(var i=0;i< this.o()['series'].length; i++){
			...
			/*}*/					
					...
					var prePonit = {};
					for(var j=0;j < s_data.length; j++){
						var x = s_data[j][0];//-筌뤿돆�녶첎占�						var y = s_data[j][3];//�⑥쥓猷�
						var y1 = s_data[j][4];//占쎈엨堉�
						var y2 = s_data[j][5];//占쎈씮��
						
						var label = s_data[j][2];//X�ル슦紐닷첎誘⑹뵬甕곤옙
			
						//if(typeof y1 != 'undefined' && typeof y2 != 'undefined' && y1 != null && y2 != null  ){
						if(typeof y1 != 'undefined' && y1 != null  ){
							...
							if(s_shape == 'arrow'){
								...
								if(typeof y1 != 'undefined'){
									this.drawArrow(px,py,y1,5,8,colorText);//占쎈뗄沅∽옙�뽰벥占쏙옙占쎈뗄沅∽옙�뽰벥占쏙옙
								}
		...
		//占쎄쑴猿쒙옙�而뽪뉩紐꺿봺疫꿸퀣肉�占싼뗫맙占쎌꼶占썸에占쏙옙�由곤옙占쏙옙�몄셽
		//this.drawLine(this.o()['axis_stroke_color'],1,x,this.o()['top_margin'],x,this.o()['top_margin']+this.getLeftAxisHeight());
		var xCntMax = this.o()['minmax']['xCntMax'];
		//var markCount = this.getMarkCountLeft(xCntMax,this.o()['leftLabelMaxHeight']);
		var markCount = this.getMarkCountLeft();
		...
		if(this.o()['xrange_axis']){
			...
			for(var i=startx; i < endx+1; i+=markGep) {
				...
				this.mainCanvasBufferCtx.fillText(x_label,
						...
						//this.o()['mark_width']+this.o()['mark_then_gep']+
						);
			...
			for(var i=startx,j=0; i < endx+1; i+=markGep,j++) {
				...
				if(groupbyNum == 1){
					//this.drawLine(this.o()['axis_stroke_color'],1,this.f(x),starty,this.f(x),this.f(starty+this.o()['mark_width']));
...
HChart.prototype.drawYAxis = function() {
	if(!this.o()['multiYAxis']){
		...
		//if((this.getLeftAxisWidth()-2) > labelLeng){
		//	this.mainCanvasBufferCtx.font = this.o()['axisTitleFont'];
		//	this.mainCanvasBufferCtx.fillStyle = this.o()['commonTextColor'];
		//	this.mainCanvasBufferCtx.fillText(this.o()['unitLabel_Verti'],
		//			this.getLeftAxisWidth()-labelLeng-this.o()['gep_space'],
		...
		*/
		...
		for(var k=0;k<axisLeng;k++){	
			if(this.o()['series'][k]['s_axis'] == 'R'){
				...
				var startLogical = this.o()['series'][k]['s_min'];
				for(var i= 0; (y = (startx-(i*markGep))) > endx-1; i++) {
							//var p_y = this.toLogicalVal_L_Y(y,this.o()['series'][k]['s_min'],this.o()['series'][k]['s_max']);
							var p_y = startLogical+i*markLogicalGep;
							var i_label;
							if(this.o()['toFixedY'] != 0){
								i_label = this.comma(chartUtil.getFixedNum(p_y,this.o()['toFixedY']));
...
HChart.prototype.getMarkCountBottom = function() {
	...
	/*
		...
		//var lmw = this.o()['bottomLabelMaxWidth']+25;//.00占쎈베猷�占쏙옙�븐늿�ㅶ묾紐꾩뵠嚥∽옙占쏙옙�먧빳占�		var lmw = this.o()['bottomLabelMaxWidth'];
		var cnt = parseInt(targetWidth/(lmw*4));
		cnt = cnt+2;//占쎌뮇�뎜ark占쏙옙占싼딆�揶쏄쑨爰쏙옙占쏙옙袁る퉸 +1占쏙옙筌띾뜄龜�귐딆벥 占싼딆�揶쏄쑨爰쏙옙占쏙옙袁る퉸 +1�곕떽占쏙옙�뤿연 +2
		var cnt_length = this.o()['minmax']['xCntMax']+2;//占쎌뮇�뎜ark占쏙옙占싼딆�揶쏄쑨爰쏙옙占쏙옙袁る퉸 +1占쏙옙筌띾뜄龜�귐딆벥 占싼딆�揶쏄쑨爰쏙옙占쏙옙袁る퉸 +1�곕떽占쏙옙�뤿연 +2
...
HChart.prototype.getMarkGepLeft = function() {
	...
	//占쎌뮇�됵옙占��븝옙苑�占쎌뮇�됵옙�곷튊占쏙옙+1),占쎌빖猷�+1
...
HChart.prototype.drawWindflag = function(x,y,_degree,width,height,_color,windspeed,_flag) {
	...
	var v2 = 0;
	var v3 = 0;
	//占쎈씮�쏙옙占퐉s占쏙옙kt揶쏉옙占쎈뜄��
	if(_flag == 'ms'){
		fval = 3;//占쎌뮇�녻퉪占쎄펾揶쏄쑨爰�
		fval2 = 2;//筌욁룂占쏙옙�뽯뻻癰귨옙瑗띶첎袁㏐봄
		v1 = 25;//1揶쏉옙占쎌눊而�
		v2 = 50;//2揶쏉옙占쎌눊而�
		v3 = 75;//3揶쏉옙占쎌눊而�
	}else{
	...
	var subLeng = width/2;//筌욁룂占쏙옙醫롮벥 疫뀀챷��獄쏆꼹�쀯쭕�깃퐨)
    this.mainCanvasBufferCtx.translate(x,y);
	this.mainCanvasBufferCtx.lineWidth = 1;
    this.mainCanvasBufferCtx.rotate(_degree*(Math.PI/180));
		this.mainCanvasBufferCtx.beginPath();
			...
			for(var i=start_ms_value,j=0; i < restVal+1;j++){
					if(j%2 == 0 ){
						base_height += flag_gep*2;//占쎌쥙占�占쎈Þ�▼첎占쏙옙�곸몵沃섓옙以�2獄쏄퀡以�占쎄쑴占�						
					}										
					this.mainCanvasBufferCtx.moveTo(0, base_height);
					if(j%2==0){
						this.mainCanvasBufferCtx.lineTo(0+width/2, base_height-subLeng/2);//width/2占쏙옙獄쏆꼹�쀯쭕�깃퐨,subLeng占쏙옙疫꿸퀣�길묾怨쀬퓗占쎌눘沅쀯옙占�					}else{
						this.mainCanvasBufferCtx.lineTo(0+width, base_height-subLeng);//占쎄쑴肉�獄쏆꼹�쀯쭕�깃퐨占쏙옙占쏙옙堉깍옙怨뚮┛占쏙옙
					}
					this.mainCanvasBufferCtx.stroke();
					if(j%2==0){
						 i+=fval2;
		...
		for(var i=0;i< this.o()['series'].length; i++){
			...
			if(s_chartType == 'spline'){
					...
					for(var j=0;j < dataLen; j++){
						...
						//var itemText = s_data[j][0]+"("+y+s_unit+"/"+percent100.toFixed(2)+"%)";
						//var itemText = s_data[j][0]+"("+y+s_unit+"/"+percent100.toFixed(2)+"%)";
		...
		if(M.getLeftAxisWidth() < lastX && lastX < (M.getLeftAxisWidth()+M.getBottomAxisWidth())){
			if(M.o()['top_margin'] < lastY && lastY < (M.o()['top_margin']+M.getLeftAxisHeight())){
				//if(M.o()['type'] != 'doughnut' && M.o()['type'] != 'pie'){
					...
					//M.f(M.getLeftAxisWidth()+this.getBottomAxisWidth()/2)
					
					//console.log(Math.tan(coord['y']/coord['x'])*Math.PI);
					
					/*
					var wd = M.conv_uv2wd(parseFloat(coord['x']),parseFloat(coord['y']));
					var ws = M.conv_uv2ws(parseFloat(coord['x']),parseFloat(coord['y']));
					(Math.PI/180)
					*/
					try{
						...
						if(tt == 'in'){
							...
							//coord_text = '        [ '+coord['x']+' , '+coord['y']+']'+'('+parseInt(degree)+'도)';
						}
...
HChart.prototype.drawSeriesRadial = function() {
	...
	try{	
		...
		for(var k=0;k<y_mark_cnt;k++){	
			...
			//this.drawLine(this.o()['axis_stroke_color'],1,this.f(orix-2),this.f(oriy-r),this.f(orix+2),this.f(oriy-r));
			

			//this.mainCanvasBufferCtx.strokeStyle
			//alert(this.mainCanvasBufferCtx.strokeStyle)
  		...
  		if(mxmi < 1 || mxmi==1){
  			...
  			for(var i=1; i< 100; i++){
  				...
  				if( mxmi <= dn_next ){
					...
					if(mxmi%divNum==0 && mxmi%5==0 && mxmi < reMax){
						reMax = mxmi;
					}
  					break;
  				}	
...
/*  		var diff;
  	...
  	chartUtil.formatData_profile2 = function(s1_data,_xMinMax,_max,_min,_divNum) {
		...
		if(_max != null && _min != null){
			...
			var o = chartUtil.getMax(maxYValue,reMin);	
			...
			if(_xMinMax){
				var reXMax = chartUtil.getMax(Number(maxXValue),Number(minXValue));			
			}
			//option.y_mark_cnt				= 6;
			...
			for(var i=10; i > 0; i-=1){
				if(m%i == 0){				
					dataInfo['y_mark_cnt'] = m/i;
					break;
				}		
			}*/
			dataInfo['y_mark_cnt'] = divNum;
			
			if(_min == null){
				var addVal = m/dataInfo['y_mark_cnt'];
				reMin -= addVal;//최소값늘리고
				dataInfo['y_mark_cnt'] +=1;
			}			
			dataInfo['maxy'] = reMax;
			dataInfo['miny'] = reMin;
			dataInfo['reArr'] = reArr;			
